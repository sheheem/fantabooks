...
class Dashboard2 {
  constructor() {
    ...
    this._horizontalTooltipChart2 = null;
  ...
  async _initCustomTooltipBar() {
    
    if (document.getElementById('horizontalTooltipChart2')) {
      const dataset2 = await axios.get('/admin/sales-by-week')
      console.log(dataset2);
      const ctx = document.getElementById('horizontalTooltipChart2').getContext('2d');
      this._horizontalTooltipChart2 = new Chart(ctx, {
        type: 'bar',
        data: {
          labels:dataset2.data.week,
          datasets: [
            {
              label: 'Sales',
              icon: 'dollar',
            ...
            yAxes: [
              {
                ...
                ticks: {
                  beginAtZero: true,
                  stepSize: Math.round((Math.max(...dataset2.data.data) - Math.min(...dataset2.data.data))/10),
                  min: 0,
                  max: Math.floor(Math.max(...dataset2.data.data))+10 ,
                  padding: 20,
                },
              },
            ],
...
class Dashboard3 {
  ...
  async _initCustomTooltipBar() {
    ...
    if (document.getElementById('horizontalTooltipChart3')) {
      const dataset3 = await axios.get('/admin/sales-by-year')
      console.log(dataset3);
      const ctx = document.getElementById('horizontalTooltipChart3').getContext('2d');
      this._horizontalTooltipChart = new Chart(ctx, {
        type: 'bar',
        data: {
          labels:dataset3.data.year,
          datasets: [
            {
              label: 'Sales',
              icon: 'dollar',
